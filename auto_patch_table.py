"""
Auto-patch script to update table body in LeadTimeDashboard.tsx
"""
import re

# Read the file
with open('web/src/pages/LeadTimeDashboard.tsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
# Pattern to match the old tbody (more flexible)
pattern = r'<tbody className="bg-white divide-y divide-gray-200">.*?</tbody>'

# Replace
new_content = re.sub(pattern, new_tbody, content, flags=re.DOTALL, count=1)

# Check if replacement happened
if new_content != content:
    # Write back
    with open('web/src/pages/LeadTimeDashboard.tsx', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("✅ Successfully updated table body!")
else:
