...
export const Inventory = () => {
  ...
  const stockTrendData = [
    { month: 'Jan', stock: 12500, target: 15000 }, { month: 'Feb', stock: 13200, target: 15000 },
    { month: 'Mar', stock: 14100, target: 15000 }, { month: 'Apr', stock: 13800, target: 15000 },
    { month: 'May', stock: 14500, target: 15000 }, { month: 'Jun', stock: 15200, target: 15000 },
  ];
  ...
  const itemColumns = [
    ...
    { key: 'current_qty' as keyof InventoryItem, header: 'Quantity', align: 'right' as const, width: '100px', sortable: true, render: (v: number, row: InventoryItem) => `${fmt(v)} ${row.uom}` },
    { key: 'current_qty_kg' as keyof InventoryItem, header: 'Weight', align: 'right' as const, width: '120px', sortable: true, render: fmtKg },
    { key: 'last_movement' as keyof InventoryItem, header: 'Last Move', width: '120px', sortable: true, render: (v: string) => new Date(v).toLocaleDateString('vi-VN') },
  ];
  ...
  if (kpisLoading || itemsLoading || plantsLoading) return <div className="flex items-center justify-center h-screen"><div className="text-lg text-slate-600">Loading inventory...</div></div>;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">Current stock levels and movements</p>
          </div>
          <DateRangePicker startDate={startDate} endDate={endDate} onDateChange={handleDateChange} />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <KPICard title="Total Items" value={fmt(kpis.total_items)} subtitle="Inventory Records" icon={<Package className="w-6 h-6" />} />
          <KPICard title="Materials" value={fmt(kpis.total_materials)} subtitle="Unique Materials" icon={<Layers className="w-6 h-6" />} />
        ...
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={stockTrendData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
              <XAxis dataKey="month" tick={{ fill: '#64748b', fontSize: 12 }} />
              <YAxis tick={{ fill: '#64748b', fontSize: 12 }} tickFormatter={(v) => `${(v/1000).toFixed(0)}K`} />
              <Tooltip contentStyle={{ backgroundColor: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px' }} />
              <Legend />
              <Line type="monotone" dataKey="stock" stroke="#3b82f6" strokeWidth={2} name="Actual Stock" dot={{ r: 4 }} />
        ...
        <div className="card">
          <h2 className="text-xl font-semibold text-slate-900 mb-4">Inventory by Plant</h2>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={byPlant} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <Tooltip contentStyle={{ backgroundColor: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px' }} />
              <Legend />
              <Bar dataKey="item_count" fill="#3b82f6" name="Item Count" radius={[8, 8, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>

        <div>
          <h2 className="text-xl font-semibold text-slate-900 mb-4">Stock by Plant</h2>
          <DataTable data={byPlant} columns={plantColumns} maxHeight="300px" />
        </div>

        <div>
          <h2 className="text-xl font-semibold text-slate-900 mb-4">Inventory Items ({items.length} records)</h2>
